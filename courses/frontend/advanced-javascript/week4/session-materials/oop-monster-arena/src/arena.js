...
export function monteCarlo(monsters, iterations = 1000) {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    champIds.forEach((id) => {
      tournamentWins[id] += 1 / champIds.length;
    });
    ids.forEach((id) => {
      boutWinTotals[id] += wins[id];
    });
  }
...
function simulateBout(a, b) {
  ...
  while (a.isAlive() && b.isAlive() && turn < 200) {
    ...
    const defenderPctAfterAttack = Math.round(
      (defenderHpAfterAttack / defender.hp.max) * 100,
    );
...
export function tournament(monsters) {
  if (monsters.length < 2) {
    ...
    if (label)
      label.textContent =
        "⚠️ Need at least 2 monsters to start the tournament!";
